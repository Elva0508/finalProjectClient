  ...
  const handleSaveComment = async (user_id, productId, orderId) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_SERVER_URL}/api/member-order-detail/comment`,
        {
          comment: comments,
          star: value,
          productid: productId,
  ...
  const getDetail = async (oid, id) => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/member-order-detail/${oid}/${id}`
      );
        ...
        <div className="d-flex py-2 justify-content-around">
          ...
          <div className="d-flex flex-column col-md-8 col-12 order-detail">
            ...
            <div className="px-md-5 px-3">
              ...
              <p className="size-7 back">
                <span>收件姓名：</span>
                {detail[0].buyer_name}
              </p>
              <p className="size-7 back">
                <span>收件地址：</span>
                {detail[0].buyer_address}
              </p>
              <p className="size-7 back">
                <span>聯絡電話：</span>
                {detail[0].buyer_phone}
              </p>
              <p className="size-7">
                <span>購買項目：</span>
              </p>
                ...
                return (
                  <div key={v.product_id}>
                    ...
                    {v.review_content !== null ? (
                      <div>
                        {showCommentForm &&
                        currentProductId === v.product_id &&
                        v.status_id === 3 ? (
                          <div className="mt-3  d-flex justify-content-end">
                            <div>
                              <Star
                                startRating={v.star_rating}
                                valid={v.star_rating}
                              />
                              <p>
                                <span>我的評論：</span>
                ...
                <table className="col-12 col-lg-4 col-md-6">
                  <thead>
                    <tr>
                      ...
                      <td className="size-7 text-end">
                        NT${detail[0].order_price}
                      </td>
                    </tr>
                    {/* <tr>

...
export default function Checkout() {
    ...
    const go = async () => {
      for (let i = 1; i < data.length + 1; i++) {
        if (commonAddress[0].city == i) {
          const newArea = data[i - 1].districts.map(
  ...
  const createOrder = async () => {
    ...
    if (sale > 0 && freight == 0) {
      newFinalCart = [
        ...finalCart,
        { product_id: 0, product_name: "優惠券", quantity: 1, newprice: -sale },
      ];
    } else if (freight > 0 && sale == 0) {
      newFinalCart = [
        ...finalCart,
        { product_id: 0, product_name: "運費", quantity: 1, newprice: freight },
      ];
  ...
  const goCheckout = () => {
    if (name === "") {
      ...
      payment == 1 &&
      (shipment == 2 || (!isChecked && shipment == 1))
    ) {
      checkout();
      router.push("/product/cart/checkout/creditCard");
      ...
      <div className="checkout mt-5">
        <div className="container">
          ...
          <div className="d-flex justify-content-center step text-center mb-4">
            <div className="col-lg-2 col-sm-4 col-5 size-6 step1">購物車</div>
          ...
          <div className="notice  mb-4 size-7 m-size-7">
            <p>
              【出貨公告】港澳地區貓砂將以多箱出貨方式。 <br />
              【出貨公告】超商取貨請填寫與證件相符之完整姓名，以確保取件權益。{" "}
              <br />
              【出貨公告】貨運配送以一樓運送為主，若有需要增加配送樓層或條件，則須另按照樓層及運送材積酌收費用。{" "}
              <br />
              【出貨公告】港澳地區恕不提供智能櫃/順豐站點配送，造成不便請見諒。
            </p>
          </div>
          {/* 收件人資訊 */}
          <div className="title size-6 mb-3 py-1 ps-2">收件人資訊</div>
                  ...
                  <div>
                    ...
                    <label for="section1">
                      ...
                      {commonAddress.length == 0 ? (
                        ...
                        <button
                          className="btn btn-confirm d-inline ms-3"
                          onClick={() => {
                            goAddress();
                          }}
                        >
                    ...
                    <div className="content">
                      ...
                      <input
                        type="checkbox"
                        className="form-check-input me-1 size-7 ms-1 mt-1"
                        checked={isChecked}
                        onChange={(e) => {
          ...
          {/* 付款方式 */}
          <div className="title size-6  mb-3 py-1 ps-2">付款方式</div>
          ...
          {/* 商品明細 */}
          <div className="title size-6  mb-3 py-1 ps-2">商品明細</div>
          {/* 購物車內商品 */}
          ...
          <div className="container py-3 col-11">
            ...
            >
              <div
                ...
                style={{ width: `${((allPrice - sale) / 1000) * 100}%` }}
              ></div>
            </div>
          </div>
        </div>
        ...
        <div className="nextStep py-2 size-7">
          <div className="container d-flex justify-content-between align-items-center">
            <button className="btn btn-brown me-auto" onClick={goPrevious}>
              上一步
            </button>

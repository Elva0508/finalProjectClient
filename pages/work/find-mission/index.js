...
const MyFilter = ({
  ...
  const handleItemClick1 = (label) => {
    // 獲取選項的value值
    const selectedValue = options1.find(
  ...
  const handleItemClick2 = (label) => {
    const selectedValue = options2.find(
      (option) => option.label === label
    )?.value;
  ...
  return (
    <>
      <div className="filters d-sm-flex justify-content-center align-items-center d-none ">
        {/* 一：任務類型 */}
        <div
          className={`btn-group mx-2 ${buttonStates.button1 ? "active" : ""}`}
        >
          <button
            ref={buttonRef1}
            className={`btn dropdown-toggle ${
              buttonStates.button1 ? "active" : ""
            }`}
            type="button"
            id="defaultDropdown1"
          ...
          <ul
            className={`dropdown-menu ${buttonStates.button1 ? "show" : ""}`}
            aria-labelledby="defaultDropdown1"
          >
            {/* 使用map函數動態生成下拉選單項 */}
            {options1.map((option) => (
              <li
        ...
        {/* 二：更新日期 */}
        <div
          className={`btn-group mx-2 ${buttonStates.button2 ? "active" : ""}`}
        >
          <button
            ref={buttonRef2}
            className={`btn dropdown-toggle ${
              buttonStates.button2 ? "active" : ""
            }`}
            type="button"
            id="defaultDropdown2"
            data-bs-toggle="dropdown"
          ...
          <ul
            className={`dropdown-menu ${buttonStates.button2 ? "show" : ""}`}
            aria-labelledby="defaultDropdown2"
          >
            {options2.map((option) => (
              <li
                key={option.label}
        ...
        {/* 城市下拉選單 */}
        <div
          className={`btn-group ms-2 ${buttonStates.button3 ? "active" : ""}`}
        >
          <button
            ref={buttonRef3}
            className={`btn dropdown-toggle ${
              buttonStates.button3 ? "active" : ""
            }`}
            type="button"
            id="defaultDropdown3"
            data-bs-toggle="dropdown"
          ...
          <ul
            className={`dropdown-menu ${buttonStates.button3 ? "show" : ""}`}
            aria-labelledby="defaultDropdown3"
          >
            {cityData.map((city) => (
              <li
                key={city.CityName}
        ...
        {selectedCity && (
          <div className={`btn-group ${buttonStates.button4 ? "active" : ""}`}>
            ...
            <ul
              className={`dropdown-menu ${buttonStates.button4 ? "show" : ""}`}
              aria-labelledby="defaultDropdown4"
            >
        ...
        <button
          className="btn-second ms-4 filter-button"
...
const LatestMission = ({ userId }) => {
  ...
  const [latestIsHovered, setLatestIsHovered] = useState(
    latestInitialHoverStates
  );
              ...
              <div className="mission-content ms-3">
                  ...
                  <div className="d-flex justify-content-between mt-1 ">
                    <div className="size-7">
                      ...
                      <div className="d-flex align-items-center mb-1">
                        ...
                        <span className="update-title">（最後更新）</span>
                      </div>
                    </div>
                ...
                <div className="d-flex justify-content-between align-items-center price">
                  ...
                  <button
                    ...
                    onMouseEnter={() => handleLatestMouseEnter(i)}
...
function ImageWithEqualDimensions({ file_path }) {
  ...
  const handleResize = () => {
    ...
    if (image) {
      // 異步操作: 可能需要時間下載圖片，導致handleResize在圖片載入之前被調用，圖片高度仍不等於寬度。增加判斷確保 handleResize 僅在 image 有值時才執行。
      // 獲取圖片的寬度
...
const MissionCard = ({
  ...
  useEffect(() => {
    if (userId) {
      // userId存在時才調用getCollection 避免依賴項userId在組件加載前還沒被設置 觸發無效請求
  ...
  const toggleCollection = async (mission_id) => {
    const isMissionInCollection = collection.some(
  ...
  return (
    <>
      {currentData.map((v, i) => {
        return (
          <motion.div
            className="col-6 col-md-4 col-lg-6 col-xl-4"
            key={v.mission_id}
            initial={"initial"}
            animate={
              missionActive === "move"
            ...
            <div className="mission-list-card ">
              ...
              <div className="mission-content mx-1 mt-2">
                  ...
                  <div className="d-flex justify-content-between mt-1 mb-1">
                    <div className="size-7">
                      ...
                      <div className="d-flex align-items-center">
                        <FaRegCalendarCheck className="me-1" />
                        {formatDate(v.update_date)}
                        <span className="update-title d-none d-sm-inline">
                          （最後更新）
                        </span>
                      </div>
                    </div>
                ...
                <div className="d-flex justify-content-between align-items-center price">
                  ...
                  <button
                    className=" heart-btn"
                    onClick={() => toggleCollection(v.mission_id)}
                    onMouseEnter={() => handleMouseEnter(i)}
                    onMouseLeave={() => handleMouseLeave(i)}
                  >
                    {collection.some(
                      (item) => item.mission_id === v.mission_id
                    ) ? (
                      <>
                        <FaHeart className="fill-icon" />
                      </>
...
export default function MissionList() {
  ...
  useEffect(() => {
    ...
    sortBy,
    search,
  ]); // 當篩選方式、排序方式發生變化時重新獲取數據（非常重要要記得！忘記好幾次）
      ...
      <div className="container my-4 find-mission">
        <nav className="breadcrumb-wrapper" aria-label="breadcrumb">
          <ol className="breadcrumb">
            ...
            <li className="breadcrumb-item" aria-current="page">
              <Link href="/work/find-mission" onClick={handleClearSettings}>
                小貓上工(找任務)
              </Link>
            </li>
            {search ? (
        ...
        <div className="d-flex flex-column flex-md-row justify-content-between mt-3">
          ...
          <Search
            ...
            setMissionCity={setMissionCity}
            setMissionArea={setMissionArea}
        ...
        <div className="d-flex justify-content-between align-items-center mt-md-3 mb-md-4 position-relative">
          ...
          <Link
            href="/work/create-mission"
            className="position-absolute add-mission-btn-pc-link"
          >
        ...
        <section className="d-flex all-mission flex-column flex-lg-row mt-2 mt-lg-3">
          ...
          <div className="latest-mission latest-mission-pc d-none d-lg-flex flex-column mb-3 position-relative">
            ...
            <img
              src="/job-icon/animation4.gif"
              className="position-absolute animation-paw"
            />
          </div>
          {/* 最新任務手機 */}
